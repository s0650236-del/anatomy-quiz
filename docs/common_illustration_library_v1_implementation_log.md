...
R04の呼吸細気管支移行部は、Phone表示で肺胞開口と側枝・切断端を一意に区別できなかった。Q040は画像参照を終了し、「壁の一部に肺胞が開口する」という形態を文章で問うtext_mcqへ変更した。R04画像は維持し、Q034・Q041・Q154・Q308の問題内容とoverlayは変更していない。

## Q231のtext_mcq化とC10利用終了

C10では心房中隔面が露出しておらず、⑥は肺動脈幹・肺動脈弁付近を指していた。画像を参照しない定義問題だったQ231はtext_mcqへ変更した。C10はQ231専用で参照0となったため、manifestとprecacheから除外し、production assetを削除した。Q230、C03、C11とその使用QIDは変更していない。

## Q064専用C12四心腔master

Q064を共有C03から、四心腔識別に特化したC12へ移行した。C12は右心房・右心室・左心房・左心室の血液腔を大きく確保し、Phone通常表示でも24px markerの周囲に内腔が残る。C03はQ017（左心室）・Q230（心室中隔）で継続利用し、画像自体と両QIDのoverlayは変更していない。

- C12：`c12_heart_four_chambers.webp`（896×1200、Q064専用）
- Q064：①右心房 `(0.2109, 0.4667)`、②右心室 `(0.3571, 0.6667)`、③左心房 `(0.6920, 0.3958)`、④左心室 `(0.6920, 0.6417)`
- 原JPEGはcrop・resizeなしでWebP（quality=92, method=6）へ変換した。
- 問題文・choices・answer・explanationは変更していない。

## 気管支問題最終整理：R07新設とQ152 text_mcq化

肺実質・肺葉・肺裂と気管支樹を同居させたR03から、気管支階層を問うQ077・Q153を専用R07へ分離した。R03のproduction画像は変更せず、Q035・Q036・Q082・Q169の肺葉・肺裂用として維持する。

- R07：`r07_bronchial_tree.webp`（896×1200、前面図、画像左が人体右）
- Q077：区域気管支の幹上 `(0.2950, 0.3660)`。問題文・choices・answer・explanationは変更していない。
- Q153：画像右側（人体左）の長い下葉気管支幹上 `(0.6663, 0.5425)`。主気管支との分岐点と区域枝分岐を避けた。問題文・choices・answer・explanationは変更していない。
- Q152：短い右主気管支へPhone用markerを置くと幹を覆うため、画像参照を終了した。「右主気管支は左より太く短く、垂直に近い」という左右差を文章で問うtext_mcqへ変更した。
- 原JPEGはcrop・resizeなしでWebP（quality=94, method=6）へ変換した。
