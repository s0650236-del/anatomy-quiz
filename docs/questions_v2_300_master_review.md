# v2 300問 統合マスターレビュー資料（Q001〜Q300）

anatomy-quiz の総論・循環器・呼吸器・泌尿器 全300問（Q001〜Q300）を1つの資料に統合し、教員・ChatGPTによる教育内容・医学的妥当性・重複・難易度・画像問題の横断レビューを可能にするための資料である。

**今回、問題内容は積極的に修正していない。** 明らかなデータ転記ミス以外は、疑問点をすべて `review_flags` として記録するにとどめ、判定・修正は教員レビューに委ねる。

- 基準HEAD: `b2ddbf328a5f94adc4323538827b94eb299b0370`（branch: v2-development）
- Q001〜Q100: `data/questions_v1.json`（実装済み）
- Q101〜Q200: `docs/questions_v2_200_expansion_review.md`（設計済み、未実装）
- Q201〜Q300: `docs/questions_v2_300_expansion_review.md`（設計済み、未実装）
- 対象カテゴリ: 総論・循環器・呼吸器・泌尿器の4領域のみ（他領域は含まない）
...
| difficulty配分 | 1: 168 / 2: 100 / 3: 32 |
| text/image | text_mcq 258 / image_mcq 42 |
| status | implemented 100 / planned 200 |
| legacy/new | legacy_revised 22 / new 178 / current_v2（既存実装） 100 |
| HIGH重複候補数 | 4グループ |
| MEDIUM重複候補数 | 25グループ |
| 数値問題数 | 10 |
| スコープ境界問題数 | 19 |
| difficulty3数 | 32（うちdifficulty_check候補 10） |
| teacher_review候補数 | 102 |
| image分類 | A: 42 / B: 48 / C: 210 |
| unique画像asset暫定数 | 22種類（既存の実在5種＋既存の未生成15種＋新規2種、後述第9節） |

### 優先して教員確認すべき問題

| 種別 | QID |
|---|---|
| HIGH重複 | Q018/Q232, Q022/Q240, Q044/Q200, Q194/Q282 |
| スコープ境界（特に内分泌・自律神経に近い） | Q149, Q185, Q188, Q189, Q190, Q239, Q243, Q269, Q283, Q294, Q296 |
| 数値・代表値（教科書間で差が出やすい） | Q114, Q171, Q191, Q228, Q237, Q278 |
| difficulty3再考候補 | Q147, Q148, Q191, Q199, Q200, Q213, Q224, Q227, Q239, Q265 |
| 医学的要確認事項あり（設計段階からの申し送り） | Q104, Q111, Q114, Q118, Q139, Q149, Q171, Q182, Q185, Q190, Q191, Q209, Q228, Q236, Q237, Q241, Q243, Q250, Q269, Q278, Q283, Q284 |
...
- image_class: C
- teacher_review: yes
- review_flags: possible_scope_boundary, textbook_variation
- 申し送り事項: 細胞小器官の微細構造は生成AIで正確に描き分けることが難しいため、text_mcqのまま運用する。
...
- image_class: C
- teacher_review: yes
- review_flags: possible_scope_boundary
...
- image_class: C
- teacher_review: yes
- review_flags: possible_semantic_overlap_medium
...
- image_class: C
- teacher_review: yes
- review_flags: textbook_variation
- 申し送り事項: 要教員確認：血液を結合組織に含めるかどうかは教科書により扱いが異なる（液状結合組織として含める立場と、独立して扱う立場がある）。採用前に確認をお願いしたい。
...
- image_class: C
- teacher_review: yes
- review_flags: possible_numeric_memorization, textbook_variation
- 申し送り事項: 体液割合の具体的数値は年齢・性別・体組成により変動する代表値であるため、出題の妥当性を教員に確認されたい。
...
- teacher_review: yes
- review_flags: textbook_variation
- 申し送り事項: 具体的な器官系名の例示は本アプリの対象範囲（総論・循環器・呼吸器・泌尿器）に含まれる系統に限定し、他の器官系名を選択肢に含めない設計とした。
...
- image_class: C
- teacher_review: yes
- review_flags: possible_scope_boundary, possible_semantic_overlap_medium
...
- teacher_review: yes
- review_flags: possible_scope_boundary, textbook_variation
- 申し送り事項: 泌尿器系の発生学的起源(中胚葉)の詳細は教科書により記述の粒度が異なるため、断定的な扱いを避け「主に」という表現にとどめた。教員確認を推奨する。
...
- image_class: C
- teacher_review: yes
- review_flags: possible_scope_boundary, difficulty_check
...
- choices:
  - A: 線維成分(コラーゲン線維など)と基質から構成され、組織に強度や弾力性を与える。
  - B: 細胞外基質は結合組織には存在しない。
  - C: 細胞外基質は上皮組織にのみ存在する。
...
- image_class: A
- teacher_review: yes
- review_flags: possible_numeric_memorization
...
- image_class: B
- teacher_review: yes
- review_flags: possible_semantic_overlap_high, image_candidate
...
- image_class: C
- teacher_review: yes
- review_flags: possible_semantic_overlap_high
...
- image_class: B
- teacher_review: yes
- review_flags: possible_semantic_overlap_medium, image_candidate
...
- tags: 心底, 心臓の位置
- image_asset: heart_exterior (新規・仮称)
- marker_target: 心底
- image_shared_with: Q138, Q140
- image_class: A
- teacher_review: no
- review_flags: なし
...
- teacher_review: yes
- review_flags: textbook_variation
- 申し送り事項: 本問は腎動脈が腹部大動脈から分岐するという解剖学的事実のみを扱い、消化器の各動脈(腹腔動脈等)の支配領域の詳細には立ち入らない設計とした。
...
- image_class: C
- teacher_review: yes
- review_flags: possible_scope_boundary, possible_semantic_overlap_medium, textbook_variation
- 申し送り事項: 自律神経の作用そのものではなく、循環器（心拍数）に対する調節結果として扱う設計とした。交感神経・副交感神経の起始部等、神経系の詳細解剖には立ち入らない。
...
- teacher_review: yes
- review_flags: possible_numeric_memorization, textbook_variation
- 申し送り事項: 心臓の大きさの目安は個人差があり代表的な説明として扱う。教員に妥当性を確認されたい。
...
- teacher_review: yes
- review_flags: textbook_variation
- 申し送り事項: 平均血圧の算出方法は簡略化した近似式が教科書によって異なるため、数式は問わず概念の理解にとどめた。
...
- teacher_review: yes
- review_flags: possible_numeric_memorization, textbook_variation
- 申し送り事項: 循環血液量の具体的割合は代表値であり、出題の妥当性を教員に確認されたい。
...
- image_class: C
- teacher_review: yes
- review_flags: possible_scope_boundary, difficulty_check, possible_semantic_overlap_medium
...
- teacher_review: yes
- review_flags: textbook_variation
- 申し送り事項: 洞房結節への血液供給には個人差があるため「多くの場合」という表現にとどめた。
...
- choices:
  - A: 交感神経と副交感神経(迷走神経)の両方が分布し、心拍数などを調節する。
  - B: 心臓には自律神経が分布しない。
  - C: 心臓には交感神経のみが分布する。
...
- teacher_review: yes
- review_flags: possible_scope_boundary, possible_semantic_overlap_medium, textbook_variation
- 申し送り事項: 自律神経の走行や起始核など神経系の詳細解剖には立ち入らず、循環器に対する分布・作用のみを扱う設計とした。
...
- choices:
  - A: 肺動脈弁は右心室と肺動脈の間、大動脈弁は左心室と大動脈の間に位置し、いずれも半月弁である。
...
- image_class: B
- teacher_review: yes
- review_flags: possible_numeric_memorization, image_candidate
...
- teacher_review: yes
- review_flags: textbook_variation
- 申し送り事項: 臨床的な起立性低血圧という病態には踏み込まず、正常な代償反射の仕組みのみを扱う設計とした。
...
- choices:
  - A: 自律神経による速やかな調節と、レニンなどのホルモンによる比較的緩やかな調節の両方が存在する。
  - B: 血圧は神経性の調節のみを受ける。
...
- image_shared_with: Q077, Q152, Q153
- image_class: A
- teacher_review: no
- review_flags: なし
...
- image_asset: q040_airway_branching.webp
- marker_target: 区域気管支
- image_shared_with: Q040, Q152, Q153
- image_class: A
- teacher_review: no
- review_flags: なし
...
- image_asset: q040_airway_branching.webp
- marker_target: 右主気管支
- image_shared_with: Q040, Q077, Q153
- image_class: A
- teacher_review: no
- review_flags: なし
...
- image_asset: q040_airway_branching.webp
- marker_target: 葉気管支
- image_shared_with: Q040, Q077, Q152
- image_class: A
- teacher_review: no
- review_flags: なし
...
- teacher_review: yes
- review_flags: possible_numeric_memorization, textbook_variation
- 申し送り事項: 基準値は年齢・状態により変動する代表値であり、出題の妥当性を教員に確認されたい。
...
- teacher_review: yes
- review_flags: possible_scope_boundary, possible_semantic_overlap_medium, textbook_variation
- 申し送り事項: 神経そのものの走行や中枢は神経系各論にあたるため扱わず、呼吸筋に対する支配関係のみを扱う設計とした。
...
- marker_target: 腎皮質
- image_shared_with: Q092, Q093, Q180
- image_class: A
- teacher_review: no
- review_flags: なし
...
- image_asset: q091_kidney_cross_section.webp
- marker_target: 腎錐体
- image_shared_with: Q091, Q093, Q180
- image_class: A
- teacher_review: no
- review_flags: なし
...
- image_asset: q091_kidney_cross_section.webp
- marker_target: 腎乳頭
- image_shared_with: Q091, Q092, Q180
- image_class: A
- teacher_review: no
- review_flags: なし
...
- image_class: B
- teacher_review: yes
- review_flags: possible_numeric_memorization, possible_semantic_overlap_medium, image_candidate
...
- image_asset: q091_kidney_cross_section.webp
- marker_target: 集合管
- image_shared_with: Q091, Q092, Q093
- image_class: A
- teacher_review: yes
- review_flags: possible_semantic_overlap_medium
...
- teacher_review: yes
- review_flags: textbook_variation
- 申し送り事項: 原尿量・再吸収率の具体的数値は教科書間で幅があるため、数値を明記せず定性的な出題にとどめた。
...
- teacher_review: yes
- review_flags: possible_scope_boundary, possible_semantic_overlap_medium, textbook_variation
- 申し送り事項: 抗利尿ホルモン自体の産生・分泌部位（下垂体）の詳細な解剖には立ち入らず、腎臓に対する作用という泌尿器側の視点にとどめる設計とした。
...
- teacher_review: yes
- review_flags: possible_scope_boundary, possible_semantic_overlap_medium, textbook_variation
- 申し送り事項: カルシウム代謝の詳細（骨の構造等）には立ち入らず、腎臓の内分泌機能としての作用のみを扱う設計とした。
...
- image_class: C
- teacher_review: yes
- review_flags: possible_numeric_memorization, difficulty_check, textbook_variation
- 申し送り事項: 腎血流量の具体的な割合は測定条件により幅があるため、代表値としての扱いを教員に確認されたい。
...
- image_class: B
- teacher_review: yes
- review_flags: difficulty_check, possible_semantic_overlap_medium, image_candidate
...
- image_class: C
- teacher_review: yes
- review_flags: difficulty_check, possible_semantic_overlap_high
...
- teacher_review: yes
- review_flags: possible_numeric_memorization, textbook_variation
- 申し送り事項: 具体的な数値は個人差がある代表値であり、教員に妥当性を確認されたい。
...
- teacher_review: yes
- review_flags: possible_scope_boundary, textbook_variation
- 申し送り事項: 傍糸球体装置の詳細な細胞構成（顆粒細胞・緻密斑等）までは看護基礎の範囲を超えるため、レニン分泌の場という概要にとどめた。
...
- teacher_review: yes
- review_flags: textbook_variation
- 申し送り事項: 具体的な基準値(mL/分等)には触れず、概念の理解にとどめた。
...
| ID群 | 内容 |
|---|---|
| Q018 / Q232 | 「左心房と左心室の間にある弁＝僧帽弁」という同一事実を、Q018は弁名を問う方向、Q232は位置を問う方向で、質問と解答を入れ替えただけで再出題している。 |
| Q022 / Q240 | 刺激伝導系の伝導順序（洞房結節→房室結節→房室束→プルキンエ線維）という同一事実を、Q022はtext、Q240はimageで問うのみで、実質的な知識の重複度が高い。 |
| Q044 / Q200 | 「右腎は肝臓の存在により左腎よりやや低い」という同一事実をほぼ同一の理由づけで再出題している。 |
| Q194 / Q282 | 腎盂の構成・役割（大腎杯からの尿を集め尿管へ送る）を、表現を変えただけでほぼ同一内容で再出題している。 |

### 2. MEDIUM重複一覧

| ID群 | 内容 |
|---|---|
| Q107 / Q207 | 筋組織の3分類（Q107）と、筋組織の共通性質＝興奮性・収縮性（Q207）。同じ「筋組織」という知識領域だが、分類と性質という異なる角度。 |
| Q065 / Q069 | 房室弁に含まれる組み合わせ（Q065）と三尖弁の位置（Q069）。同じ房室弁の知識だが、分類か個別位置かで角度が異なる。 |
| Q066 / Q070 | 半月弁に含まれる組み合わせ（Q066）と肺動脈弁の位置（Q070）。同上のパターン。 |
| Q069 / Q232 / Q245 | Q245（三尖弁・僧帽弁の位置関係統合）は、既存Q069と新設Q232の事実をそのまま組み合わせた統合問題であり、新規知識は少ない。 |
| Q070 / Q233 / Q246 | Q246（肺動脈弁・大動脈弁の位置関係統合）も同様に、既存Q070と新設Q233の再組み合わせ。 |
| Q027 / Q073 / Q074 / Q140 / Q252 | Q252（冠循環全体の統合）は、既存の冠状動脈・冠状静脈洞問題群をまとめて再言及する構成。 |
| Q149 / Q243 | 心拍数の自律神経調節（機能面、Q149）と心臓への自律神経分布（構造面、Q243）。近接した知識領域。 |
| Q023 / Q024 / Q068 / Q144 | Q144（心電図波形の統合）は、既存のP波・QRS群・T波問題をそのまま時系列でつなげた構成。 |
| Q133 / Q225 | 血圧を規定する要因（心拍出量×末梢血管抵抗、Q133）と、末梢血管抵抗を規定する要因（細動脈半径、Q225）。親子関係にある近接知識。 |
| Q188 / Q239 / Q251 | Q251（血圧の調節機構統合）は、既存の圧受容器（Q239）とレニン（Q188）を組み合わせた構成。 |
| Q085 / Q175 / Q274 | 呼吸器系の構造分類（Q085）、導気部から呼吸部への移行（Q175）、上気道・下気道の区分（Q274）。いずれも呼吸器系の構造的二分法を扱う類似した総括問題。 |
| Q038 / Q083 / Q170 | Q170（横隔膜の復習）は、既存Q038・Q083の内容をほぼそのまま再確認する構成。 |
| Q083 / Q269 | 吸息時の呼吸筋調節（横隔神経、Q083）と呼吸筋の神経支配（横隔神経+肋間神経、Q269）。Q269はQ083に肋間神経を追加したのみ。 |
| Q038 / Q157 / Q158 / Q170 / Q262 / Q273 | Q273（呼吸筋の使い分け統合）は、既存の吸気筋・呼気筋・呼吸補助筋問題群をまとめて再言及する構成。 |
| Q039 / Q083 / Q164 / Q269 / Q277 | Q277（呼吸調節の全体像統合）は、既存の呼吸中枢・呼吸筋調節・化学受容器・神経支配をまとめて再言及する構成。 |
| Q162 / Q172 / Q276 | Q276（外気から組織までの酸素の流れ統合）は、既存のガス交換の駆動力（Q172）と酸素運搬（Q162）を組み合わせた構成。 |
| Q099 / Q297 | 腎臓の位置（脊椎レベル、Q099）と腎臓の位置と体表指標（肋骨、Q297）。同じ「腎臓の位置」知識だが着眼点が異なる。 |
| Q196 / Q279 | 腎臓の被膜構造（線維被膜・脂肪被膜、Q196）に、Q279（腎筋膜）が層を1つ追加する構成。 |
| Q181 / Q177 / Q183 / Q180 / Q185 / Q298 | Q298（ネフロンの機能統合復習）は、既存のネフロン各部の機能問題群をまとめて対応づける構成。 |
| Q199 / Q298 | ネフロンの流れの順序（Q199）と各部位の機能対応（Q298）。同じ5部位の知識を、順序か機能かという異なる角度で問う。 |
| Q185 / Q188 / Q294 | Q294（尿量調節ホルモンの統合）は、既存のADH（Q185）とレニン（Q188）をそのまま組み合わせた構成。 |
| Q186 / Q190 / Q295 | Q295（電解質・ミネラル調節統合）は、既存のNa+調節（Q186）とビタミンD（Q190）をそのまま組み合わせた構成。 |
| Q187 / Q296 | 腎臓による酸塩基平衡調節（Q187）に、Q296が呼吸器との役割分担を追加する構成。 |
| Q090 / Q197 | 排尿時の状態（排尿筋収縮等、Q090）と排尿時の神経性調節（副交感神経、Q197）。近接した知識領域。 |
| Q089 / Q090 / Q197 / Q198 | Q198（蓄尿・排尿の統合）は、既存の蓄尿・排尿状態問題と神経性調節問題をまとめて再言及する構成。 |

### 3. 数値問題一覧

| ID | category | 数値・代表値 | 暗記価値 | 条件依存性 | textbook variationの可能性 |
|---|---|---|---|---|---|
| Q016 | 循環器 | 左第5肋間・鎖骨中線付近 | 高（触診の基本） | 低 | 低（標準的記述） |
| Q081 | 呼吸器 | 気管の長さ　約10cm | 中 | 高（体格差で変動） | 中（9〜15cm等、幅のある記載あり） |
| Q099 | 泌尿器 | 腎臓位置　第12胸椎〜第3腰椎 | 中 | 中（体格・呼吸相で変動） | 中（記載範囲に差あり） |
| Q114 | 総論 | 体液（水分）割合　約60% | 高（基礎生理の代表値） | 高（年齢・性別・体組成で20〜80%まで変動） | 高（60%/55%等、教科書で数値が分かれる） |
| Q171 | 呼吸器 | 呼吸数　12〜18回/分 | 高（バイタルサインの基礎） | 中（年齢・状態で変動） | 中（12〜20回等、幅の記載が異なる） |
| Q191 | 泌尿器 | 腎血流量　心拍出量の約1/4 | 中 | 中 | 中（20〜25%等の幅表現あり） |
| Q228 | 循環器 | 心臓の大きさ　にぎりこぶし大 | 低〜中（比喩表現） | 高（個人差大） | 高（比喩表現自体が教科書間で一定しない） |
| Q237 | 循環器 | 循環血液量　体重の約8%（約80mL/kg） | 高（輸液・出血量計算の基礎） | 中 | 中（7〜8%等の幅表現あり） |
| Q248 | 循環器 | 心尖・心底の位置（Q016と同一の5肋間・鎖骨中線） | 中（Q016との重複再確認） | 低 | 低 |
| Q278 | 泌尿器 | 腎臓の大きさ　握りこぶし大・約10cm | 低〜中 | 高（個人差） | 高（表現が教科書間で一定しない） |

### 4. スコープ境界一覧

4領域から外れかけている候補。ただし腎臓とADH・RAAS、呼吸調節、循環調節など、4領域の正常生理を理解するために直接必要なものは直ちにスコープ外とは判定せず「境界候補」として提示するにとどめる。

| ID | category | 内容 |
|---|---|---|
| Q104 | 総論 | 細胞小器官（ミトコンドリア）は総論の範囲内だが、生化学寄りの内容でありスコープ境界として留意。 |
| Q106 | 総論 | 体細胞分裂は総論の範囲内だが、発生学・細胞生物学寄りの内容。 |
| Q108 | 総論 | 神経組織の役割は総論の基本組織分類の一部として扱っているが、神経系各論との境界に留意。 |
| Q109 | 総論 | グリア細胞は神経組織の構成要素として総論に位置づけたが、神経系各論の入り口に近い内容。 |
| Q149 | 循環器 | 心拍数の自律神経調節。循環器の正常生理として直接必要な内容だが、自律神経（神経系）に関する記述を含む。 |
| Q185 | 泌尿器 | 抗利尿ホルモン（下垂体後葉ホルモン）の腎臓への作用。泌尿器の正常生理として直接必要だが、内分泌（下垂体）に関する記述を含む。 |
| Q188 | 泌尿器 | レニン-アンジオテンシン系。泌尿器の正常生理として直接必要だが、内分泌系の反応系列を含む。 |
| Q189 | 泌尿器 | エリスロポエチン。腎臓の内分泌機能として直接必要だが、血液学（赤血球産生）と接続する内容。 |
| Q190 | 泌尿器 | 活性型ビタミンD。腎臓の内分泌機能として直接必要だが、内分泌・骨代謝と接続する内容。 |
| Q207 | 総論 | 筋組織の共通性質（興奮性・収縮性）。総論の範囲内だが、運動器・神経系にまたがる横断的概念。 |
| Q208 | 総論 | 興奮性（活動電位）。総論の範囲内だが、神経生理学の基礎概念に踏み込んでいる。 |
| Q209 | 総論 | 呼吸器・泌尿器の発生学的由来（胚葉）。総論の発生の範囲内だが、発生学的な内容。 |
| Q213 | 総論 | 電解質と興奮性の関係。総論の範囲内だが、神経生理学に接続する内容。 |
| Q239 | 循環器 | 圧受容器による血圧調節。循環器の正常生理として直接必要だが、自律神経反射を含む。 |
| Q243 | 循環器 | 心臓に分布する自律神経。循環器の正常生理として直接必要だが、自律神経（神経系）の記述を含む。 |
| Q269 | 呼吸器 | 呼吸筋の神経支配（横隔神経・肋間神経）。呼吸器の正常生理として直接必要だが、末梢神経の名称を含む。 |
| Q283 | 泌尿器 | 傍糸球体装置とレニン分泌。泌尿器の内分泌機能として直接必要だが、内分泌の細胞レベルの内容。 |
| Q294 | 泌尿器 | ADH・RAASの統合。泌尿器の正常生理として直接必要だが、複数のホルモン系にまたがる。 |
| Q296 | 泌尿器 | 腎臓と呼吸器の酸塩基平衡統合。両領域の正常生理として直接必要。 |

### 5. difficulty3一覧

| ID | category | subcategory | difficulty_check |
|---|---|---|---|
| Q131 | 循環器 | 心臓の機能 | なし |
| Q142 | 循環器 | 刺激伝導系 | なし |
| Q144 | 循環器 | 心電図 | なし |
| Q146 | 循環器 | 肺循環 | なし |
| Q147 | 循環器 | 体循環 | 体循環の経路を順にたどるだけの記述であり、統合的推論というより既知事項の整理に近い。difficulty2程度の可能性。 |
| Q148 | 循環器 | 肺循環 | 肺循環の経路を順にたどるだけの記述であり、同上の理由でdifficulty2程度の可能性。 |
| Q172 | 呼吸器 | ガス交換 | なし |
| Q175 | 呼吸器 | 呼吸器系の構造分類 | なし |
| Q181 | 泌尿器 | 尿生成 | なし |
| Q187 | 泌尿器 | 尿生成 | なし |
| Q191 | 泌尿器 | 尿生成 | 「心拍出量の約1/4」という単一の数値知識であり、複数知識の統合とまでは言えない可能性。 |
| Q198 | 泌尿器 | 排尿反射 | なし |
| Q199 | 泌尿器 | ネフロン | ネフロンの各部位名を順に並べるだけの記述であり、difficulty2（構造と機能の関連付け）程度の可能性。 |
| Q200 | 泌尿器 | 腎臓の位置 | Q044と同一事実（右腎が肝臓により低い）の再言及であり、Q044がdifficulty1であることと整合しない可能性。 |
| Q208 | 総論 | 筋組織 | なし |
| Q211 | 総論 | 体腔 | なし |
| Q213 | 総論 | 体液 | 電解質と興奮性の関係を一文で述べるのみであり、真の統合というより単一事実の記述に近い可能性。 |
| Q223 | 総論 | 生体の恒常性 | なし |
| Q224 | 循環器 | 心臓の機能 | 後負荷の定義そのものであり、統合的推論を要さない可能性。 |
| Q227 | 循環器 | 血管 | 細動脈の役割の定義そのものであり、統合的推論を要さない可能性。 |
| Q234 | 循環器 | 冠状動脈 | なし |
| Q239 | 循環器 | 血圧 | 圧受容器の存在と機序を一文で述べるのみであり、difficulty2程度の可能性。 |
| Q247 | 循環器 | 心臓の機能 | なし |
| Q250 | 循環器 | 血圧 | なし |
| Q251 | 循環器 | 血圧 | なし |
| Q265 | 呼吸器 | ガス交換 | 換気血流比の定義そのものであり、difficulty2程度の可能性。 |
| Q273 | 呼吸器 | 呼吸筋 | なし |
| Q276 | 呼吸器 | ガス交換 | なし |
| Q277 | 呼吸器 | 呼吸調節 | なし |
| Q286 | 泌尿器 | ネフロン | なし |
...
| ID | category | review_flags |
|---|---|---|
| Q016 | 循環器 | possible_numeric_memorization |
| Q018 | 循環器 | possible_semantic_overlap_high, image_candidate |
| Q022 | 循環器 | possible_semantic_overlap_high |
| Q023 | 循環器 | possible_semantic_overlap_medium |
| Q024 | 循環器 | possible_semantic_overlap_medium |
| Q027 | 循環器 | possible_semantic_overlap_medium, image_candidate |
| Q038 | 呼吸器 | possible_semantic_overlap_medium |
| Q039 | 呼吸器 | possible_semantic_overlap_medium |
| Q044 | 泌尿器 | possible_semantic_overlap_high, image_candidate |
| Q065 | 循環器 | possible_semantic_overlap_medium, image_candidate |
| Q066 | 循環器 | possible_semantic_overlap_medium, image_candidate |
| Q068 | 循環器 | possible_semantic_overlap_medium |
| Q069 | 循環器 | possible_semantic_overlap_medium, image_candidate |
| Q070 | 循環器 | possible_semantic_overlap_medium, image_candidate |
| Q073 | 循環器 | possible_semantic_overlap_medium |
| Q074 | 循環器 | possible_semantic_overlap_medium |
| Q081 | 呼吸器 | possible_numeric_memorization |
| Q083 | 呼吸器 | possible_semantic_overlap_medium |
| Q085 | 呼吸器 | possible_semantic_overlap_medium, image_candidate |
| Q089 | 泌尿器 | possible_semantic_overlap_medium |
| Q090 | 泌尿器 | possible_semantic_overlap_medium |
| Q099 | 泌尿器 | possible_numeric_memorization, possible_semantic_overlap_medium, image_candidate |
| Q104 | 総論 | possible_scope_boundary, textbook_variation |
| Q106 | 総論 | possible_scope_boundary |
| Q107 | 総論 | possible_semantic_overlap_medium |
| Q108 | 総論 | possible_scope_boundary |
| Q109 | 総論 | possible_scope_boundary |
| Q111 | 総論 | textbook_variation |
| Q114 | 総論 | possible_numeric_memorization, textbook_variation |
| Q118 | 総論 | textbook_variation |
| Q133 | 循環器 | possible_semantic_overlap_medium |
| Q139 | 循環器 | textbook_variation |
| Q140 | 循環器 | possible_semantic_overlap_medium |
| Q144 | 循環器 | possible_semantic_overlap_medium |
| Q147 | 循環器 | difficulty_check |
| Q148 | 循環器 | difficulty_check |
| Q149 | 循環器 | possible_scope_boundary, possible_semantic_overlap_medium, textbook_variation |
| Q157 | 呼吸器 | possible_semantic_overlap_medium |
| Q158 | 呼吸器 | possible_semantic_overlap_medium |
| Q162 | 呼吸器 | possible_semantic_overlap_medium |
| Q164 | 呼吸器 | possible_semantic_overlap_medium |
| Q170 | 呼吸器 | possible_semantic_overlap_medium |
| Q171 | 呼吸器 | possible_numeric_memorization, textbook_variation |
| Q172 | 呼吸器 | possible_semantic_overlap_medium |
| Q175 | 呼吸器 | possible_semantic_overlap_medium |
| Q177 | 泌尿器 | possible_semantic_overlap_medium |
| Q180 | 泌尿器 | possible_semantic_overlap_medium |
| Q181 | 泌尿器 | possible_semantic_overlap_medium |
| Q182 | 泌尿器 | textbook_variation |
| Q183 | 泌尿器 | possible_semantic_overlap_medium |
| Q185 | 泌尿器 | possible_scope_boundary, possible_semantic_overlap_medium, textbook_variation |
| Q186 | 泌尿器 | possible_semantic_overlap_medium |
| Q187 | 泌尿器 | possible_semantic_overlap_medium |
| Q188 | 泌尿器 | possible_scope_boundary, possible_semantic_overlap_medium |
| Q189 | 泌尿器 | possible_scope_boundary |
| Q190 | 泌尿器 | possible_scope_boundary, possible_semantic_overlap_medium, textbook_variation |
| Q191 | 泌尿器 | possible_numeric_memorization, difficulty_check, textbook_variation |
| Q194 | 泌尿器 | possible_semantic_overlap_high |
| Q196 | 泌尿器 | possible_semantic_overlap_medium |
| Q197 | 泌尿器 | possible_semantic_overlap_medium |
| Q198 | 泌尿器 | possible_semantic_overlap_medium |
| Q199 | 泌尿器 | difficulty_check, possible_semantic_overlap_medium, image_candidate |
| Q200 | 泌尿器 | difficulty_check, possible_semantic_overlap_high |
| Q207 | 総論 | possible_scope_boundary, possible_semantic_overlap_medium |
| Q208 | 総論 | possible_scope_boundary |
| Q209 | 総論 | possible_scope_boundary, textbook_variation |
| Q213 | 総論 | possible_scope_boundary, difficulty_check |
| Q224 | 循環器 | difficulty_check |
| Q225 | 循環器 | possible_semantic_overlap_medium |
| Q227 | 循環器 | difficulty_check |
| Q228 | 循環器 | possible_numeric_memorization, textbook_variation |
| Q232 | 循環器 | possible_semantic_overlap_high |
| Q233 | 循環器 | possible_semantic_overlap_medium |
| Q236 | 循環器 | textbook_variation |
| Q237 | 循環器 | possible_numeric_memorization, textbook_variation |
| Q239 | 循環器 | possible_scope_boundary, difficulty_check, possible_semantic_overlap_medium |
| Q240 | 循環器 | possible_semantic_overlap_high |
| Q241 | 循環器 | textbook_variation |
| Q243 | 循環器 | possible_scope_boundary, possible_semantic_overlap_medium, textbook_variation |
| Q245 | 循環器 | possible_semantic_overlap_medium, image_candidate |
| Q246 | 循環器 | possible_semantic_overlap_medium, image_candidate |
| Q248 | 循環器 | possible_numeric_memorization, image_candidate |
| Q250 | 循環器 | textbook_variation |
| Q251 | 循環器 | possible_semantic_overlap_medium |
| Q252 | 循環器 | possible_semantic_overlap_medium |
| Q262 | 呼吸器 | possible_semantic_overlap_medium |
| Q265 | 呼吸器 | difficulty_check |
| Q269 | 呼吸器 | possible_scope_boundary, possible_semantic_overlap_medium, textbook_variation |
| Q273 | 呼吸器 | possible_semantic_overlap_medium |
| Q274 | 呼吸器 | possible_semantic_overlap_medium, image_candidate |
| Q276 | 呼吸器 | possible_semantic_overlap_medium |
| Q277 | 呼吸器 | possible_semantic_overlap_medium |
| Q278 | 泌尿器 | possible_numeric_memorization, textbook_variation |
| Q279 | 泌尿器 | possible_semantic_overlap_medium, image_candidate |
| Q282 | 泌尿器 | possible_semantic_overlap_high |
| Q283 | 泌尿器 | possible_scope_boundary, textbook_variation |
| Q284 | 泌尿器 | textbook_variation |
| Q294 | 泌尿器 | possible_scope_boundary, possible_semantic_overlap_medium |
| Q295 | 泌尿器 | possible_semantic_overlap_medium |
| Q296 | 泌尿器 | possible_scope_boundary, possible_semantic_overlap_medium |
| Q297 | 泌尿器 | possible_semantic_overlap_medium, image_candidate |
| Q298 | 泌尿器 | possible_semantic_overlap_medium, image_candidate |

### 7. image A一覧（画像がほぼ必須）

| ID | category | marker_target |
|---|---|---|
| Q002 | 総論 | 細胞→組織→器官→器官系→個体の各段階 |
| Q004 | 総論 | 最表層の上皮組織 |
| Q008 | 総論 | 冠状面 |
| Q011 | 総論 | 最外層（外胚葉） |
| Q016 | 循環器 | 心尖 |
| Q017 | 循環器 | 左心室 |
| Q019 | 循環器 | 肺から左心房へ入る肺静脈 |
| Q021 | 循環器 | 毛細血管 |
| Q025 | 循環器 | 右心房上部の洞房結節 |
| Q032 | 呼吸器 | 喉頭 |
| Q035 | 呼吸器 | 右肺中葉 |
| Q037 | 呼吸器 | 肺胞から血液へ向かう矢印 |
| Q040 | 呼吸器 | 呼吸細気管支 |
| Q041 | 呼吸器 | 肺胞嚢 |
| Q045 | 泌尿器 | 腎門 |
| Q048 | 泌尿器 | 腎小体 |
...
| Q180 | 泌尿器 | 集合管 |
| Q192 | 泌尿器 | 膀胱 |
| Q240 | 循環器 | ①洞房結節→②房室結節→③房室束→④プルキンエ線維の順にoverlaysを設定 |

### 8. image B一覧（画像化すると教育効果がかなり高い）

| ID | category | subcategory | question |
|---|---|---|---|
| Q013 | 循環器 | 循環器系の構成 | 循環器系に含まれる組み合わせとして正しいものはどれか。 |
| Q018 | 循環器 | 心臓弁 | 左心房と左心室の間にある弁はどれか。 |
| Q020 | 循環器 | 血管 | 静脈の特徴として適切なのはどれか。 |
| Q026 | 循環器 | 大動脈 | 大動脈弓から直接分岐する血管はどれか。 |
| Q027 | 循環器 | 冠状動脈 | 右冠状動脈と左冠状動脈が起始する部位はどれか。 |
| Q031 | 呼吸器 | 気道 | 上気道に含まれるものはどれか。 |
| Q033 | 呼吸器 | 咽頭 | 軟口蓋より上方に位置する咽頭の区分はどれか。 |
| Q036 | 呼吸器 | 肺葉 | 左右の肺葉数の組み合わせとして正しいものはどれか。 |
| Q044 | 泌尿器 | 腎臓の位置 | 腎臓について正しいものはどれか。 |
| Q065 | 循環器 | 心臓弁 | 房室弁に含まれる組み合わせはどれか。 |
| Q066 | 循環器 | 心臓弁 | 半月弁に含まれる組み合わせはどれか。 |
| Q069 | 循環器 | 心臓弁 | 三尖弁の位置として正しいものはどれか。 |
| Q070 | 循環器 | 心臓弁 | 肺動脈弁の位置として正しいものはどれか。 |
| Q079 | 呼吸器 | 下気道 | 下気道に含まれるものはどれか。 |
| Q085 | 呼吸器 | 呼吸器系の構造分類 | 呼吸器系の構造分類として正しいものはどれか。 |
| Q087 | 呼吸器 | 呼吸部 | 呼吸部に含まれるものはどれか。 |
| Q096 | 泌尿器 | 膀胱三角 | 膀胱三角について正しいものはどれか。 |
| Q099 | 泌尿器 | 腎臓の位置 | 腎臓の位置として正しいものはどれか。 |
| Q100 | 泌尿器 | 尿路 | 尿の通り道として正しいものはどれか。 |
| Q122 | 総論 | 身体の大区分 | 腹部の体表区分について正しいものはどれか。 |
| Q126 | 循環器 | 心膜 | 心膜について正しいものはどれか。 |
| Q128 | 循環器 | 心臓の内部構造 | 左心室と右心室の壁の厚さについて正しいものはどれか。 |
| Q137 | 循環器 | 血管 | 毛細血管での物質交換について正しいものはどれか。 |
| Q150 | 循環器 | 心臓の内部構造 | 心臓における心房と心室の位置関係として正しいものはどれか。 |
| Q168 | 呼吸器 | 胸膜 | 縦隔について正しいものはどれか。 |
| Q195 | 泌尿器 | 尿路 | 尿管の走行について正しいものはどれか。 |
| Q199 | 泌尿器 | ネフロン | ネフロンにおける原尿(尿)の流れとして正しい順序はどれか。 |
| Q210 | 総論 | 体腔 | 体腔とその内容の組み合わせとして正しいものはどれか。 |
| Q217 | 総論 | 身体の大区分 | 胸部の体表からの区分について正しいものはどれか。 |
| Q226 | 循環器 | 血管 | 微小循環について正しいものはどれか。 |
| Q229 | 循環器 | 心臓の位置 | 解剖学的正位における心臓の向きについて正しいものはどれか。 |
| Q230 | 循環器 | 心臓の内部構造 | 心室中隔について正しいものはどれか。 |
| Q245 | 循環器 | 心臓弁 | 三尖弁・僧帽弁の位置関係の統合として正しいものはどれか。 |
| Q246 | 循環器 | 心臓弁 | 肺動脈弁・大動脈弁の位置関係の統合として正しいものはどれか。 |
| Q248 | 循環器 | 心臓の位置 | 心臓の位置についての統合的理解として正しいものはどれか。 |
| Q254 | 呼吸器 | 上気道 | 副鼻腔について正しいものはどれか。 |
| Q256 | 呼吸器 | 喉頭 | 喉頭蓋の機能について正しいものはどれか。 |
| Q257 | 呼吸器 | 気管 | 気管の走行について正しいものはどれか。 |
| Q258 | 呼吸器 | 気管 | 気管の壁の構造について正しいものはどれか。 |
| Q259 | 呼吸器 | 気管支 | 気管分岐部について正しいものはどれか。 |
| Q260 | 呼吸器 | 肺葉 | 肺門について正しいものはどれか。 |
| Q274 | 呼吸器 | 呼吸器系の構造分類 | 上気道・下気道の区分として正しいものはどれか。 |
| Q279 | 泌尿器 | 腎臓の外形 | 腎臓の被膜構造について、既存の線維被膜・脂肪被膜に加えて正しいものはどれか。 |
| Q280 | 泌尿器 | 腎臓の内部構造 | 腎柱について正しいものはどれか。 |
| Q281 | 泌尿器 | 腎臓の内部構造 | 小腎杯・大腎杯について正しいものはどれか。 |
| Q297 | 泌尿器 | 腎臓の位置 | 腎臓の位置と体表指標の関係について正しいものはどれか。 |
| Q298 | 泌尿器 | ネフロン | ネフロンの各部位とその主なはたらきの組み合わせとして正しいものはどれか。 |
| Q299 | 泌尿器 | 尿路 | 泌尿器系全体の構成について正しいものはどれか。 |

### 9. 画像共有asset一覧

| asset | 実在 | 使用候補QID |
|---|---|---|
| q002_hierarchy.webp | 実在 | Q002 |
| q004_epithelium.webp | 実在 | Q004 |
| q017_heart_chambers.webp | 実在 | Q017, Q064, Q129 |
| q037_alveolar_gas_exchange.webp | 実在 | Q037 |
| q048_nephron.webp | 実在 | Q048, Q176 |
| q008_body_planes.webp | 未生成 | Q008, Q053, Q054 |
| q011_germ_layers.webp | 未生成 | Q011 |
| q016_apex.webp | 未生成 | Q016 |
| q019_pulmonary_vein.webp | 未生成 | Q019 |
| q021_vessel_cross_sections.webp | 未生成 | Q021, Q067 |
| q025_conduction_system.webp | 未生成 | Q025, Q240 |
| q032_larynx.webp | 未生成 | Q032 |
| q035_right_middle_lobe.webp | 未生成 | Q035, Q082, Q169 |
| q040_airway_branching.webp | 未生成 | Q040, Q077, Q152, Q153 |
| q041_alveolar_sac.webp | 未生成 | Q041, Q154 |
| q045_renal_hilum.webp | 未生成 | Q045 |
| q050_urinary_system.webp | 未生成 | Q050, Q192 |
| q068_ecg_waveform.webp | 未生成 | Q068 |
| q073_coronary_arteries.webp | 未生成 | Q073, Q074 |
| q091_kidney_cross_section.webp | 未生成 | Q091, Q092, Q093, Q180 |
| heart_exterior (新規・仮称) | 未生成 | Q125, Q138, Q140 |
| pleura_cross_section (新規・仮称) | 未生成 | Q155 |

unique asset暫定数：**22種類**（実在5種＋未生成17種）で、image_mcq 42問すべてをカバーする設計。
...
多いこと自体を誤りとはしないが、10問以上のsubcategory（心臓弁12、尿生成17、ネフロン10、腎臓の内部構造15など）は、教員レビュー時に優先的に目を通すことを推奨する。
